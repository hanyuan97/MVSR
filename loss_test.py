...
def init(args):
    ...
    return net, net_f, cri_gan, loss_fn_g, optimizer_g, training_loader
...
def train(EPOCH, training_loader, log_file, args):
...
def train_one_epoch(epoch_index, is_gan=False):
    ...
    for data, real_H in training_loader:
        ...
        if is_gan:
            ...
            log_file.write(f"{loss_pix.item()},{loss_pix.item()*pix_w},{loss_fea.item()},{loss_g.item()}\n")
        # loss_g.backward()
        # optimizer_g.step()
...
if __name__ == "__main__":
    ...
    for i in range(10, 11):
        ...
        train(args.epoch, training_loader, log_file, args)
    log_file.close()
    #if args.save:
